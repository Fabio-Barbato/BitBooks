...
int notify_user(const char* username, int client_socket) {
    ...
    cJSON *notifications_to_send = cJSON_CreateArray(); // Inizializzo ad ogni chiamata

    // Accumula le notifiche dell'utente
    ...
    cJSON_Delete(json); // Libera il JSON principale

    // Controlla se ci sono notifiche da inviare
    if (cJSON_GetArraySize(notifications_to_send) > 0) {
        char *notifications_to_send_str = cJSON_Print(notifications_to_send); // Stampa JSON
        send(client_socket, notifications_to_send_str, strlen(notifications_to_send_str), 0);
        free(notifications_to_send_str); // Libera la stringa
    } else {
        // Invia un JSON vuoto se non ci sono notifiche
        char* response_str = strdup("[]"); 
        send(client_socket, response_str, strlen(response_str), 0);
        free(response_str); 
    }

    cJSON_Delete(notifications_to_send); // Libera l'array delle notifiche
    return 0;
}
